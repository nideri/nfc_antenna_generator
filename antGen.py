...
c_drillSize               = -1        # [mm]  - drill size for pad holes in mm (diameter). if set to value < 0, drillSize is set to floor(conductorWidth/2*10)/10
...
c_style                   = 3         # [#]   - how should the antenna look like
                                      #           1) see doc/st.png or doc/ant_style_1.png
...
def genAnt( turns=3,        # [#]   
  ...
  if drillSize < 0:
      ...
      if drillSize == 0:
          print("WARNING: drillSize was < 0 and is therefore autocalculated. the resulted drillSize is 0! select drillSize manually!")
  ...
  if style == 3: # const 45 deg slope
  ...
  elif style == 2: # do it like in niceAnt.png
    if condSpaceMin<0:
    ...
    for seg in range(0,turns*6-1):
        ...
        y0 = y1
    mod="%s    ))\n" % (mod)
    mod="%s  (pad 2 thru_hole circle (at %f %f) (size %f %f) (drill %f) (layers *.Cu *.Mask))\n" % (mod, antLength/2-xg/2+x0, antWidth-condWidth/2+y0, condWidth, condWidth, drillSize)
  else: # do it like in doc/st.png
    ...
    for seg in range(0,turns*4):
        ...
        y0 = y1
    mod="%s    ))\n" % (mod)
    mod="%s  (pad 2 thru_hole circle (at %f %f) (size %f %f) (drill %f) (layers *.Cu *.Mask))\n" % (mod, condWidth/2+x0, antWidth-condWidth/2+y0, condWidth, condWidth, drillSize)
  mod="%s)\n" % (mod)
  return mod
...
def main(args):
    ...
    if len(args) > 0:
        ...
        parser.add_argument("-d", "--drillSize"            , type=float,             help="drill size for pad holes in mm (diameter). if set to value < 0, drillSize is set to floor(conductorWidth/2*10)/10", required=False)
